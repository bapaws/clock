...
public extension CKRecordConvertible where Self: Object {
        ...
        for prop in properties {
            ...
            if prop.isArray {
                ...
                case .object:
                    ...
                    guard let list = (item as? RLMSwiftCollectionBase)?._rlmCollection, list.count > 0 else { break }
                    var referenceArray = [CKRecord.Reference]()
                    let wrappedArray = list
                    for index in 0 ..< wrappedArray.count {
                        ...
                        case .objectId:
                            if let primaryValueObjectId = object[primaryKey] as? ObjectId, let obj = object as? CKRecordConvertible, !obj.isDeleted {
                                let referenceZoneID = CKRecordZone.ID(zoneName: "\(object.objectSchema.className)sZone", ownerName: CKCurrentUserDefaultName)
                                referenceArray.append(CKRecord.Reference(recordID: CKRecord.ID(recordName: primaryValueObjectId.stringValue, zoneID: referenceZoneID), action: .none))
                            }
                        default:
                            break
                        }
            ...
            case .objectId:
                // _id is a system field
                let name = prop.name == "_id" ? "object_id__" : prop.name
                r[name] = (item as? ObjectId)?.stringValue
            case .object:
